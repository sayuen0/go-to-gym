...
func (r *userR) GetTrainingRecords() TrainingRecordSlice {
	if r == nil {
		return nil
	}
	return r.TrainingRecords
}
...
func (o *User) Exercises(mods ...qm.QueryMod) exerciseQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`exercises`.`user_id`=?", o.ID),
	)

	return Exercises(queryMods...)
}

// TrainingRecords retrieves all the training_record's TrainingRecords with an executor.
func (o *User) TrainingRecords(mods ...qm.QueryMod) trainingRecordQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`training_records`.`user_id`=?", o.ID),
	)
...
func (userL) LoadExercises(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`exercises`),
		qm.WhereIn(`exercises.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadTrainingRecords allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadTrainingRecords(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`training_records.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load training_records")
	}

	var resultSlice []*TrainingRecord
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice training_records")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on training_records")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.TrainingRecords = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.TrainingRecords = append(local.R.TrainingRecords, foreign)
				if foreign.R == nil {
					foreign.R = &trainingRecordR{}
				}
				foreign.R.User = local
...
// AddTrainingRecords adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.TrainingRecords.
// Sets related.R.User appropriately.
func (o *User) AddTrainingRecords(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TrainingRecord) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			TrainingRecords: related,
		}
	} else {
		o.R.TrainingRecords = append(o.R.TrainingRecords, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &trainingRecordR{
				User: o,
			}
		} else {
			rel.R.User = o
